...
Re-run color extraction with custom settings. Colors are already extracted automatically on upload — use this only to force a refresh or change the number of extracted colors. The extraction runs asynchronously; this endpoint queues the job and returns immediately. Poll `GET /colors/images/{image_id}` to check when results are ready.
...
Poll `GET /colors/images/{image_id}` until `status` is `"completed"`.
...
Re-run color extraction for multiple images with custom settings. Colors are already extracted automatically on upload.
...
Find images that contain a specific color. Provide at least one of `hex_code`, `color_name`, or `color_family` — all three are optional fields and can be combined.
...
`match_score` is a normalized similarity score from 0 to 1 (higher = closer match), computed as `max(0, 1 − (delta_e / delta_e_threshold))`.
...
[
  {
    ...
    "example_colors": ["#FFFFFF", "#808080", "#000000", "#F5F5DC"]
  },
  {
    "name": "earth_tone",
    "display_name": "Earth Tones",
    "description": "Warm, natural colors inspired by earth and nature",
    "example_colors": ["#C4A87C", "#8B4513", "#D2691E", "#A0522D"]
  },
  {
    "name": "warm",
    "display_name": "Warm Colors",
    "description": "Reds, oranges, yellows - energizing and inviting",
    "example_colors": ["#FF6347", "#FFA500", "#FFD700", "#DC143C"]
  },
  {
    "name": "cool",
    "display_name": "Cool Colors",
    "description": "Blues, greens, purples - calming and sophisticated",
    "example_colors": ["#4169E1", "#008080", "#2E8B57", "#9370DB"]
  },
  {
    "name": "metallic",
    "display_name": "Metallic Tones",
    "description": "Metallic and shimmer colors — gold, silver, bronze, copper",
    "example_colors": ["#FFD700", "#B87333", "#C0C0C0", "#B5A642"]
  },
  {
    "name": "pastel",
    "display_name": "Pastels",
    "description": "Soft, muted colors with white undertones",
    "example_colors": ["#FFB6C1", "#ADD8E6", "#98FB98", "#FFDAB9"]
  },
  {
    "name": "vibrant",
    "display_name": "Vibrant Colors",
    "description": "Bold, highly saturated colors",
    "example_colors": ["#FF1493", "#00FF00", "#FF4500", "#8A2BE2"]
  }
]
```
