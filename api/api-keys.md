...
|-----------|------|---------|-------------|
| include_inactive | boolean | `false` | Include inactive (revoked) keys in results |
...
> **Reserved metadata keys**: The following keys are managed by the system and cannot be set or updated: `created_by_user_id`, `deactivation_reason`, `deactivated_at`. Attempting to set them returns `400 Bad Request`.
...
|-----------|------|-------------|
| reason | string | Optional reason for revocation, stored in the key's audit metadata |
...
Resource-specific scopes use the format `{resource}:{permission}`.

**Resources**: `api_keys`, `usage`, `tenants`, `cloud_storage`
