...
**Errors**: `409 Conflict` — a folder with that name already exists at this level · `400 Bad Request` — validation error
...
**Errors**: `409 Conflict` — a folder with that name already exists at this level · `404 Not Found` — folder not found
...
**Errors**: `404 Not Found` — folder or target parent not found · `409 Conflict` — a folder with that name already exists at the destination · `422 Unprocessable Entity` — move would create a circular reference, or would push the subtree beyond the 10-level depth limit
...
| `delete_all` | The folder and all its contents (subfolders and files) are deleted and cannot be recovered via the API |
