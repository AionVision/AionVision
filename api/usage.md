...
> **Note:** The Python SDK does not have a dedicated `client.usage` resource. Call these endpoints directly via HTTP. For upload quota specifically, use `client.uploads.check_quota()`.
...
| GET | /usage/breakdown | Credit usage breakdown for the current billing period |
...
The `plan` object mirrors the plan limits section and is always present. Deprecated `overage` fields (`supports_overage`, `overage_credits_used`, etc.) may appear in the response but are always `false`/`0` and can be ignored.
...
```
```python
# No SDK wrapper — call the REST API directly
import httpx
async with httpx.AsyncClient() as http:
    resp = await http.get(
        "https://api.aionvision.tech/api/v2/usage/summary",
        params={"period": "month"},
...
| operation_type | string | Filter by operation type (e.g. `describe`, `upload`) |
  ...
  "events": [
    {
      ...
      "operation_type": "chat",
      "endpoint": "/api/v2/chat/sessions/{id}/messages",
      "method": "POST",
      "status_code": 200,
...
The response also includes `prepaid_credits` and several deprecated overage fields (`supports_overage`, `included_credits`, `overage_credits_used`, `max_overage_credits`, `overage_cost`) that are always `false`/`0` and can be ignored.
...
async with httpx.AsyncClient() as http:
    resp = await http.post(
        "https://api.aionvision.tech/api/v2/usage/check",
        json={"item_count": 10},
        headers={"Authorization": f"Bearer {api_key}"},
    )
    check = resp.json()
...
Credit usage summary for the current billing month including totals, limit, remaining credits, and the next reset date.
...
- SDK: No dedicated usage resource yet — use direct HTTP calls (see examples above). For upload quota, use `client.uploads.check_quota()`.
