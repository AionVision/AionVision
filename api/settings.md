...
| POST | /tenant/members/invite | Invite a new member | ADMIN |
| PATCH | /tenant/members/{user_id}/role | Change member role | ADMIN |
| DELETE | /tenant/members/{user_id} | Remove a member | ADMIN |
...
Rate limited: 3 requests per day. Sends a confirmation email to the new address. The email address is not changed until the user clicks the confirmation link.
...
| password | string | Yes | Current password (security verification) |

**Response** `202 Accepted` — email change pending confirmation.

### DELETE /user/sessions/{session_id}

Revoke a specific session by ID (from `GET /user/sessions`). The current session cannot be revoked.

**Response** `204 No Content`

### POST /user/revoke-all-sessions
...
```json
{
  ...
  "created_at": "2025-01-01T00:00:00Z",
  "updated_at": "2025-06-15T08:22:00Z",
  "is_active": true,
  "webhook_url": null,
...
Returns a **flat JSON array** (not wrapped in an object). Requires VIEW permission.
...
[
  {
    ...
    "last_active_at": "2025-06-15T14:22:00Z"
  },
  {
...
> **Note:** The response is a raw array, not `{"members": [...]}`. Parse the response body directly as a list.
...
Invite a new member. Creates a user account if the email doesn't exist. Requires ADMIN or OWNER. Rate limited: 5/min.
...
Valid roles: `"viewer"`, `"editor"`, `"admin"` (cannot assign `"owner"`)
...
**Response** `200 OK`
```json
{"removed": true, "message": "Custom S3 configuration removed"}
```

If no custom S3 was configured, `removed` is `false` and message reflects that.
