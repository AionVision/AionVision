...
AionVision is an Agentic Vision AI platform that provides agentic infrastructure, AI-powered image and video understanding, document processing, semantic search, agentic vision, file management, and more. This repository contains the public API specification and Python SDK reference for building applications with Aionvision.

> **Note on embeddings:** AionVision automatically generates multiple embedding types (feature embeddings, visual embeddings, color embeddings, etc.) for images, videos, and documents behind the scenes. These power semantic search, agent operations, and chat — but are not directly exposed through the API. You interact with the results (search, chat, pipelines) rather than the embeddings themselves.
...
- Directory options: `recursive=True` (default) traverses subdirectories; `include_hidden=False` (default) skips dotfiles and hidden directories
...
Upload videos with automatic chunking, AI scene detection, and analysis.
- Single upload: `client.video_uploads.upload_one("video.mp4")`
- Batch upload: `client.video_uploads.upload(["a.mp4", "b.mov"])` or `client.video_uploads.upload("/path/to/videos/")`
- Supported formats: `.mp4`, `.mov`, `.m4v`, `.webm`, `.avi`
- Progress callbacks: `on_chunk_progress`, `on_upload_complete`, `on_analysis_progress`
- Scene retrieval: `client.video_scenes.list(video_id)`, `client.video_scenes.get_thumbnail(scene_id)`
- Analysis management: `client.video_analysis.get_status(job_id)`, `client.video_analysis.wait_for_completion(job_id)`

See: [examples/uploading_videos.py](examples/uploading_videos.py)
...
`send_stream()` yields typed `ChatToken` events. Handle by `event.type` (`ChatTokenType` enum):
- `token` — LLM response token; `event.content` (current), `event.data["accumulated"]` (full text so far)
- `thinking_step` — AI reasoning step; `event.data["phase"]`, `event.data["message"]`
- `status` — agent progress update; `event.data["message"]`
- `image_results` — images found by search agents; `event.data["images"]`, `event.data["count"]`
- `document_results` — documents found by search agents; `event.data["documents"]`, `event.data["count"]`
- `tool_invocation` / `tool_result` — agent tool activity
- `plan_pending_approval` — execution plan awaiting user approval; `event.data["plan_id"]`
- `complete` — final response; `event.data["content"]`, `event.data["processing_time_ms"]`, etc.
- `connection` / `ping` / `close` — connection lifecycle
- `error` — streaming error; `event.data["message"]`, `event.data["error_type"]`
...
- `client.agent_search.videos("construction site overview")`
...
Manage tenant settings, team members, and usage limits (requires ADMIN or OWNER role).
- `client.tenant.get_settings()` — get tenant configuration and subscription info
- `client.tenant.update_settings(name=..., allowed_vlm_providers=...)` — update tenant config
- `client.tenant.get_limits()` — get usage limits and remaining quota
- `client.tenant.list_members()` — list team members (returns a flat list)
- `client.tenant.invite_member(email, role="editor")` — invite a new member
- `client.tenant.update_member_role(user_id, "admin")` — change a member's role
- `client.tenant.remove_member(user_id)` — remove a member

See: [examples/tenant_admin.py](examples/tenant_admin.py), [api/settings.md](api/settings.md)

### Audit Logs
Query admin and security event logs for compliance monitoring (requires ADMIN role).
- `client.audit.list(event_type=..., severity=..., date_from=...)` — list filtered audit events
- `client.audit.get(log_id)` — get a single audit event

See: [examples/tenant_admin.py](examples/tenant_admin.py), [api/settings.md](api/settings.md)

### Custom S3 Storage
Store uploaded files in your own AWS S3 bucket instead of the default Aionvision bucket.
- `client.settings.configure_custom_s3(access_key_id, secret_access_key, bucket_name, region)`
- `client.settings.get_custom_s3_status()` — check configuration status
- `client.settings.validate_custom_s3()` — test that stored credentials still work
- `client.settings.remove_custom_s3()` — revert to default storage
...
from aion.exceptions import (
    ...
    VideoUploadError,        # Video upload failed - has media_id, stage, partial_chunks
    VideoAnalysisError,      # Video analysis failed - has media_id, analysis_job_id, status
...
| `succeeded_count` | `int` | Number of results with `description_status == "completed"` |
| `failed_count` | `int` | Number of results with `description_status == "failed"` |
| `pending_count` | `int` | Number of results still pending/processing |
| `succeeded()` | `list[UploadResult]` | All results with completed descriptions |
| `failed()` | `list[UploadResult]` | All results with failed descriptions |
...
> **Note:** `succeeded_count`, `failed_count`, and `pending_count` track **description** status, not upload status. With `wait_for_descriptions=False`, all results will have `description_status == "pending"`, so `succeeded_count` will be `0` even though all files uploaded successfully. Use `len(results)` to count uploads.
...
| `video_metadata` | `dict \| None` | Video-specific metadata (duration, resolution, codecs). When present, individual fields within the dict (e.g. `duration_seconds`, `file_size_bytes`) may be `None` if not yet extracted. |
...
History of processing operations on a file.
| Field | Type | Description |
|-------|------|-------------|
...
Different resources use different pagination parameters:

| Resource | Parameters | Total field | Response list field |
|----------|-----------|-------------|---------------------|
| Files (`client.files.list()`) | `limit` + `offset` | `total_count` | `files` |
| Documents (`client.documents.list()`) | `page` + `page_size` | `total_count` | `documents` |
| Chat sessions (`client.chats.list_sessions()`) | `limit` + `offset` | `total` | `items` |

All list endpoints return `has_more: bool` to indicate whether more results exist.

### Auto-paginating iterators

Files, documents, and links provide a `list_all()` async iterator that handles pagination automatically, yielding one item at a time:

```python
# Iterate through all files without manual pagination
async for file in client.files.list_all(media_types=["image"], page_size=50):
    print(file.title)

# Also available on documents and links
async for doc in client.documents.list_all(page_size=50):
    print(doc.filename)

async for link in client.links.list_all(page_size=50):
    print(link.url)
```

For manual pagination control, use `list()` instead and manage `offset`/`page` yourself.
...
| [Uploads](api/uploads.md) | Stream upload images (single and batch), quota checking |
...
| [Videos](api/videos.md) | Chunked multipart upload, AI scene detection, video analysis |
