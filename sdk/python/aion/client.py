...
from .resources.video_scenes import VideoScenesResource
from .resources.video_uploads import VideoUploadsResource
...
class AionVision:
    ...
    async def describe(self, image: Optional[Union[str, Path, bytes, list[Union[str, Path, bytes]]]] = None, *, object_key: Optional[str] = None, object_keys: Optional[list[str]] = None, include_metadata: bool = True, include_tags: bool = True, providers: Optional[list[str]] = None, prompt: Optional[str] = None, rule_set_id: Optional[str] = None, rules: Optional[list[dict[str, Any]]] = None, max_parallel: int = 5, timeout_per_item: int = 30, on_progress: Optional[Callable[[BatchStatusResult], None]] = None) -> Union[DescriptionResult, list[DescriptionResult]]:
        """
                ...
                Args:
                    ...
                    object_key: Storage object key for single previously uploaded image
                    object_keys: List of Storage object keys for batch processing
    ...
    async def verify(self, image: Optional[Union[str, Path, bytes]] = None, content: str = '', *, images: Optional[list[Union[str, Path, bytes]]] = None, contents: Optional[list[str]] = None, object_key: Optional[str] = None, object_keys: Optional[list[str]] = None, mode: str = 'response', verification_level: str = 'standard', use_consensus: bool = True, providers: Optional[list[str]] = None, rule_set_id: Optional[str] = None, rules: Optional[list[dict[str, Any]]] = None, max_parallel: int = 5, timeout_per_item: int = 30, on_progress: Optional[Callable[[BatchStatusResult], None]] = None) -> Union[VerificationResult, list[VerificationResult]]:
        """
                ...
                Args:
                    ...
                    object_key: Storage object key for single previously uploaded image
                    object_keys: List of Storage object keys for batch verification
    ...
    @property
    def video_uploads(self) -> VideoUploadsResource:
        """

                Access video upload operations.

                Provides high-level convenience methods:
                - upload_one() - Upload a single video with automatic chunking
                - upload() - Upload multiple videos or a directory of videos

                And low-level methods for fine-grained control:
                - initiate() - Start a chunked upload session
                - batch_initiate() - Start a batch upload (up to 10 videos)
                - confirm_chunk() - Confirm a chunk after upload
                - complete() - Finalize upload and trigger analysis
                - abort() - Cancel an in-progress upload
                - get_progress() - Check chunk-level upload progress
                - retry_chunk() - Get a new presigned URL for a failed chunk

                Supported formats: .mp4, .mov, .m4v, .webm, .avi

                Example:
                    ```python
                    # Simple single video upload
                    result = await client.video_uploads.upload_one("video.mp4")
                    print(result.media_id, result.analysis_job_id)

                    # Upload with progress callback and wait for analysis
                    result = await client.video_uploads.upload_one(
                        "video.mp4",
                        wait_for_analysis=True,
                        on_chunk_progress=lambda e: print(f"{e.progress_percent:.0f}%"),
                    )
                    ```
        """
        ...
    ...
    @property
    def video_analysis(self) -> VideoAnalysisResource:
        """
                ...
                Example:
                    ```python
                    status = await client.video_analysis.get_status(job_id)
                    if not status.is_terminal:
                        final = await client.video_analysis.wait_for_completion(job_id)
                    ```
        """
        ...

    @property
    def video_scenes(self) -> VideoScenesResource:
        """
                ...
                Example:
                    ...
                    for scene in scenes.scenes:
                        print(scene.time_range_formatted, scene.description)
                    ```
        """
        ...
