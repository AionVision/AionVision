...
from .types.agent_search import DocumentSearchAgentResult, ImageSearchAgentResult, VideoSearchAgentResult
from .types.callbacks import VideoAnalysisProgressEvent, VideoChunkProgressEvent, VideoUploadCompleteEvent
from .types.video import BatchVideoUploadSession, ChunkConfirmResult, ChunkRetryResult, VideoAbortResult, VideoAnalysisJobStatus, VideoAnalysisQueueResult, VideoAnalysisRetryResult, VideoAnalysisStatus, VideoChunkInfo, VideoMetadata, VideoScene, VideoSceneList, VideoUploadProgress, VideoUploadResult, VideoUploadSession, VideoUploadStatus
...
from .types.upload import BatchUploadResults, QuotaInfo, UploadResult
...
class SyncChatSession:
    ...
    def send(self, message: str, *, force_detailed_analysis: bool = False, expected_image_ids: Optional[list[str]] = None) -> ChatResponse:
        """
                ...
                Args:
                    ...
                    expected_image_ids: Image IDs the session expects to have. Used to
                        synchronize context when images were recently added.
    ...
    def update_videos(self, video_ids: list[str]) -> dict[str, Any]:
        """

                Update the videos associated with this session.

                Args:
                    video_ids: List of video IDs to associate with the session
...
class SyncFilesResource(_SyncResourceBase):
    ...
    def list(self, *, search: Optional[str] = None, search_mode: str = 'all', tags: Optional[list[str]] = None, date_from: Optional[datetime] = None, date_to: Optional[datetime] = None, has_description: Optional[bool] = None, ids: Optional[list[str]] = None, media_types: Optional[list[str]] = None, limit: int = 20, offset: int = 0, sort_by: str = 'content_created_at', sort_order: str = 'desc') -> FileList:
        """List files with search/filter/pagination."""
        ...

    def list_all(self, *, search: Optional[str] = None, search_mode: str = 'all', tags: Optional[list[str]] = None, date_from: Optional[datetime] = None, date_to: Optional[datetime] = None, has_description: Optional[bool] = None, media_types: Optional[list[str]] = None, sort_by: str = 'content_created_at', sort_order: str = 'desc', page_size: int = 50) -> Iterator[UserFile]:
        """
...
class SyncChatResource(_SyncResourceBase):
    ...
    def send(self, message: str, *, session_id: str, force_detailed_analysis: bool = False, expected_image_ids: Optional[list[str]] = None) -> ChatResponse:
        """Send a message and get a complete response."""
        ...
    ...
    def update_videos(self, session_id: str, video_ids: list[str]) -> dict[str, Any]:
        """Update videos in a session."""
        ...
...
class SyncAgentSearchResource(_SyncResourceBase):
    ...
    def videos(self, query: str, *, limit: int = 50, folder_id: Optional[str] = None, video_ids: Optional[list[str]] = None) -> VideoSearchAgentResult:
        """Execute AI-powered video search."""
        ...
...
class SyncDocumentsResource(_SyncResourceBase):
    ...
    def confirm_upload(self, object_key: str, size_bytes: int, content_type: str, *, checksum: Optional[str] = None) -> DocumentConfirmResult:
        """Confirm a document upload after uploading the file."""
        ...
    ...
    def batch_confirm(self, batch_id: str, confirmations: list[dict[str, Any]]) -> DocumentBatchConfirmResult:
        """Confirm batch upload after uploading the files."""
        ...
...
class SyncVideoUploadsResource(_SyncResourceBase):
    """Synchronous wrapper for video upload operations."""
    ...
    def get_progress(self, media_id: str) -> VideoUploadProgress:
        """Get upload progress."""
        ...
    ...
    def retry_chunk(
        ...
        """Get a new URL for retrying a failed chunk."""
        ...
...
class SyncVideoAnalysisResource(_SyncResourceBase):
    """Synchronous wrapper for video analysis operations."""

    def get_status(self, analysis_job_id: str) -> VideoAnalysisJobStatus:
        """Get analysis job status."""
        ...

    def get_status_by_media(self, media_id: str) -> VideoAnalysisJobStatus:
        """Get analysis status by media ID."""
        ...
...
class SyncVideoScenesResource(_SyncResourceBase):
    """Synchronous wrapper for video scene operations."""

    def list(self, video_id: str) -> VideoSceneList:
...
class SyncAionVision:
    ...
    from .types.common import ChatTokenType, DescriptionStatus, MessageRole, StorageTarget
    ...
    from .types.agent_search import DocumentChunkResultItem, DocumentSearchAgentResult, ImageSearchAgentResult, ImageSearchResultItem, MatchedSceneItem, ResultRefData, VideoSearchAgentResult, VideoSearchResultItem
    ...
    @property
    def video_uploads(self) -> SyncVideoUploadsResource:
        """

                Access video upload operations with native sync interface.

                Example:
                    result = client.video_uploads.upload_one("video.mp4")
                    results = client.video_uploads.upload("/path/to/videos")
        """
        ...

    @property
    def video_analysis(self) -> SyncVideoAnalysisResource:
        """

                Access video analysis operations with native sync interface.

                Example:
                    status = client.video_analysis.get_status(job_id)
                    final = client.video_analysis.wait_for_completion(job_id)
        """
        ...

    @property
    def video_scenes(self) -> SyncVideoScenesResource:
        """

                Access video scene data with native sync interface.

                Example:
                    scenes = client.video_scenes.list(video_id)
                    thumbnail = client.video_scenes.get_thumbnail(scene_id)
        """
        ...
