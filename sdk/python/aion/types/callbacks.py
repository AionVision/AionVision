...
if TYPE_CHECKING:
    ...
    from .video import VideoAnalysisJobStatus, VideoUploadResult
...
@dataclass(frozen=True)
class VideoChunkProgressEvent:
    """

        Event fired after each video chunk is uploaded.

        Attributes:
            chunk_number: One-based chunk number of the uploaded chunk
...
@dataclass(frozen=True)
class VideoUploadCompleteEvent:
    """

        Event fired when a video upload completes.

        Attributes:
            media_id: Video identifier
            result: The complete upload result
    """
    media_id: str
...
class VideoAnalysisProgressEvent:
    """
        ...
        Event fired as video analysis progresses during polling.

        Attributes:
            job_status: Current analysis job status
    """
    job_status: VideoAnalysisJobStatus

    @property
    def progress_percent(self) -> float:
        """Calculate analysis progress as percentage (0-100)."""
        ...

    @property
    def is_terminal(self) -> bool:
        """Whether the analysis has reached a final state."""
        ...
