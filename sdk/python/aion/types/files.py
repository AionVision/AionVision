...
class UserFileDetails:
    """
        ...
        Attributes:
            ...
            media_type: Computed type: 'image', 'video', 'document', or 'link'
            dimensions: Image dimensions {width, height}
            format: Image format
            ...
            has_audio_transcript: Whether audio transcript exists
            video_url: URL to original playable video file (for videos only)
            document_type: Document format type (pdf, docx, txt, etc.)
            ...
            document_url: URL to original document file
            source_url: Original URL (for links)
            domain: Domain of the link
            og_metadata: Open Graph metadata dict
            favicon_url: Favicon URL
            page_title: Page title from OG/HTML
            crawl_status: Link crawl status (pending/queued/processing/completed/failed)
            crawl_error: Error message if crawl failed
            crawled_at: When the link was last crawled
            extracted_images: Images extracted from the crawled page
    ...
    og_metadata: Optional[dict[str, Any]] = None
