...
class ChatSessionContext:
    ...
    async def send(self, message: str, *, force_detailed_analysis: bool = False, expected_image_ids: Optional[list[str]] = None) -> ChatResponse:
        """
                ...
                Args:
                    ...
                    expected_image_ids: Image IDs the session expects to have.
                        If any are missing, the API auto-heals the session context.
    ...
    async def send_stream(self, message: str, *, force_detailed_analysis: bool = False, expected_image_ids: Optional[list[str]] = None) -> AsyncIterator[ChatToken]:
        """
    ...
    async def update_videos(self, video_ids: list[str]) -> dict[str, Any]:
        """Update selected videos for this session's context."""
        ...
...
class ChatResource:
    ...
    async def send(self, message: str, *, session_id: str, force_detailed_analysis: bool = False, expected_image_ids: Optional[list[str]] = None) -> ChatResponse:
        """
    ...
    async def send_stream(self, message: str, *, session_id: str, force_detailed_analysis: bool = False, expected_image_ids: Optional[list[str]] = None) -> AsyncIterator[ChatToken]:
        """
    ...
    async def update_videos(self, session_id: str, video_ids: list[str]) -> dict[str, Any]:
        """

                Update selected videos for session context.

                Sets which videos are available for the VideoAnalysisAgent
                in the chat session. Maximum 100 videos.

                Args:
                    session_id: Session identifier
                    video_ids: Video IDs to use as context

                Returns:
                    Update result with session_id, selected_video_ids, video_count
        """
        ...
