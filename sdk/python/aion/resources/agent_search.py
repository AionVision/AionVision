...
class AgentSearchResource:
    """
        ...
        Provides programmatic access to ImageSearchAgent, DocumentSearchAgent,
        and VideoSearchAgent with full ReAct reasoning capabilities.
    ...
    async def videos(self, query: str, *, limit: int = 50, folder_id: Optional[str] = None, video_ids: Optional[list[str]] = None) -> VideoSearchAgentResult:
        """

                Execute AI-powered video search.

                Searches across videos and their individual scenes. Supports
                browse mode (list videos) and content search (find scenes
                matching a query with timestamps).

                Args:
                    query: Natural language search query
                    limit: Maximum results (1-500, default 50)
                    folder_id: Restrict search to specific folder
                    video_ids: Restrict search to specific videos

                Returns:
                    VideoSearchAgentResult with:
                    - results: Video objects with matched scenes
                    - video_ids: Flat list of video IDs
                    - summary: Human-readable summary
                    - result_refs: For interactive UI building
        """
        ...
