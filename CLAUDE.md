...
- **OpenAPI spec**: `openapi.json` - 164 paths, 251 schemas
...
| `upload_batch(files, *, filenames, recursive=True, include_hidden=False, wait_for_descriptions=True, raise_on_failure=True, description_timeout, on_progress, on_file_complete, on_description_progress, on_description_failed, storage_target="default") -> BatchUploadResults` | Lower-level batch upload (always auto-describes at default level) |
...
| `send(message, *, session_id, force_detailed_analysis=False, expected_image_ids=None) -> ChatResponse` | Send message (complete response) |
| `send_stream(message, *, session_id, force_detailed_analysis=False, expected_image_ids=None) -> AsyncIterator[ChatToken]` | Stream response tokens |
...
| `update_videos(session_id, video_ids) -> dict` | Update session video context (max 100) |
...
| `search_images(query, *, limit=50, offset=0) -> list[ImageReference]` | Search images for chat context |
...
| Method | Description |
|--------|-------------|
| `send(message, *, force_detailed_analysis=False, expected_image_ids=None) -> ChatResponse` | Send message in session |
| `send_stream(message, *, force_detailed_analysis=False, expected_image_ids=None) -> AsyncIterator[ChatToken]` | Stream in session |
| `update_images(image_ids) -> dict` | Update image context |
| `update_documents(document_ids) -> dict` | Update document context |
| `update_videos(video_ids) -> dict` | Update video context |
...
| Method | Description |
|--------|-------------|
| `list(*, search, search_mode, tags, date_from, date_to, has_description, ids, media_types, limit=20, offset=0, sort_by, sort_order) -> FileList` | List files with filtering. `media_types`: `["image"]`, `["video"]`, `["document"]`, `["link"]` or any combination |
| `list_all(*, search, search_mode, tags, date_from, date_to, has_description, media_types, sort_by, sort_order, page_size=50) -> AsyncIterator[UserFile]` | Iterate all files |
...
| `upload(files, *, filenames, recursive=True, include_hidden=False, wait_for_processing=True, on_progress, on_file_complete, on_processing_progress, on_processing_failed, storage_target) -> BatchDocumentUploadResults` | Upload multiple documents |
...
| `videos(query, *, limit=50, folder_id, video_ids) -> VideoSearchAgentResult` | AI video search |
...
### client.video_uploads (VideoUploadsResource)

| Method | Description |
|--------|-------------|
| `upload_one(file, *, title, tags, auto_analyze=True, wait_for_analysis=False, analysis_timeout, analysis_type="full_vlm", max_chunk_concurrency, max_chunk_retries=3, on_chunk_progress, on_upload_complete, on_analysis_progress) -> VideoUploadResult` | Upload single video with automatic chunking. Handles full lifecycle. Supported formats: .mp4 .mov .m4v .webm .avi |
| `upload(files, *, title, tags, auto_analyze=True, wait_for_analysis=False, analysis_timeout, analysis_type, max_chunk_concurrency, max_chunk_retries=3, on_chunk_progress, on_upload_complete, on_analysis_progress) -> list[VideoUploadResult]` | Upload one or more videos. Directories expand to supported video files. |
| `initiate(filename, content_type, size_bytes, *, title, tags, metadata) -> VideoUploadSession` | Low-level: start chunked upload, get presigned chunk URLs |
| `batch_initiate(videos, *, auto_process=True, processing_priority="normal") -> BatchVideoUploadSession` | Low-level: start batch upload (up to 10 videos) |
| `confirm_chunk(media_id, upload_id, chunk_number, etag, size_bytes) -> ChunkConfirmResult` | Low-level: confirm a chunk after upload |
| `complete(media_id, upload_id, *, duration_seconds, duration_extraction_failed, auto_analyze=True, analysis_type="full_vlm", title, tags, metadata) -> VideoUploadResult` | Low-level: finalize upload and trigger analysis |
| `abort(media_id, upload_id, *, reason) -> VideoAbortResult` | Cancel an in-progress upload and clean up resources |
| `get_progress(media_id) -> VideoUploadProgress` | Check chunk-level upload progress |
| `retry_chunk(media_id, upload_id, chunk_number) -> ChunkRetryResult` | Get a new presigned URL for a failed chunk |

### client.video_analysis (VideoAnalysisResource)

| Method | Description |
|--------|-------------|
| `get_status(analysis_job_id) -> VideoAnalysisJobStatus` | Get analysis job status by job ID |
| `get_status_by_media(media_id) -> VideoAnalysisJobStatus` | Get latest analysis status by video ID |
| `retry(analysis_job_id, media_id, *, reason) -> VideoAnalysisRetryResult` | Retry a failed analysis job |
| `queue(media_id, *, analysis_type="full_vlm", priority="normal") -> VideoAnalysisQueueResult` | Queue a video for analysis |
| `wait_for_completion(analysis_job_id, *, timeout, on_progress) -> VideoAnalysisJobStatus` | Poll until analysis completes (exponential backoff, max 15s interval) |

### client.video_scenes (VideoScenesResource)

| Method | Description |
|--------|-------------|
| `list(video_id) -> VideoSceneList` | Get all AI-detected scenes for a video |
| `get_thumbnail(scene_id) -> bytes` | Download scene thumbnail (raw JPEG bytes) |
...
### Video Upload + Scene Extraction
```python
# Upload and wait for AI analysis to complete
result = await client.video_uploads.upload_one(
    "footage.mp4",
    wait_for_analysis=True,
    on_chunk_progress=lambda e: print(f"Upload: {e.progress_percent:.0f}%"),
    on_analysis_progress=lambda e: print(f"Analysis: {e.progress_percent:.0f}%"),
)
print(result.media_id, result.analysis_job_id)

# Retrieve extracted scenes
scenes = await client.video_scenes.list(result.media_id)
for scene in scenes.scenes:
    print(f"{scene.time_range_formatted}: {scene.description}")
```
...
except AionVision.VideoUploadError as e:
    # e.media_id — video ID (if initiate succeeded)
    # e.stage — "initiate", "chunk_upload", "confirm", or "complete"
    # e.partial_chunks — chunks uploaded before failure
except AionVision.VideoAnalysisError as e:
    # e.media_id, e.analysis_job_id, e.status
```
...
- **UserFile**: id, title, filename, tags, upload_description, visible_text, created_at, content_created_at, size_bytes, has_full_description, thumbnail_url, medium_url, full_url, content_type, media_type, dimensions, format, variant_status, variant_count, blur_hash, description_status, description_error, video_metadata, video_analysis_status, video_analysis_job_id, scene_count, has_audio_transcript, video_url
- **UserFileDetails**: id, object_key, title, original_filename, tags, upload_description, visible_text, full_descriptions (list[FullDescription] — plural, not `.full_description`), processing_history (list[ProcessingHistory]), full_url, medium_url, thumbnail_url, original_url, dimensions, format, size_bytes, content_type, media_type, hash, description_generated_at, created_at, updated_at, content_created_at, upload_method, variant_status, variant_count, blur_hash, description_status. Additional fields by media_type — video: `video_metadata`, `video_analysis_status`, `video_analysis_job_id`, `scene_count`, `has_audio_transcript`, `video_url`; document: `document_type`, `page_count`, `text_extraction_status`, `chunk_count`, `document_url`; link: `source_url`, `domain`, `og_metadata`, `favicon_url`, `page_title`, `crawl_status`, `crawl_error`, `crawled_at`, `extracted_images`, `extracted_images_count`
...
- **ChatSessionDetail**: session (ChatSession — title is nested: `detail.session.title`), messages (list[ChatMessage]), selected_image_ids, selected_video_ids, current_search_result_ids
...
- **StepResult**: agent, status ("completed"/"failed"/"skipped"), summary, outputs (dict), error, execution_time_ms, node_id (str | None — pipeline node identifier from API)
...
- **VideoUploadResult**: media_id, status, metadata (VideoMetadata | None), thumbnail_url, video_url, analysis_job_id, processing_status, created_at. Properties: is_completed (bool), is_analyzing (bool)
- **VideoMetadata**: duration_seconds, resolution, width, height, frame_rate, video_codec, audio_codec, bitrate_kbps, file_size_bytes, format_name
- **VideoUploadSession**: media_id, upload_id, chunks (list[VideoChunkInfo]), total_chunks, chunk_size_bytes, expires_at, storage_path, concurrent_upload_limit, retry_attempts
- **VideoChunkInfo**: chunk_number, size_bytes, upload_url, expires_at, upload_method
- **ChunkConfirmResult**: chunk_number, status, chunks_completed, total_chunks, progress_percent, estimated_time_remaining
- **VideoUploadProgress**: media_id, upload_id, status, chunks_completed, total_chunks, bytes_uploaded, total_bytes, progress_percent, failed_chunks (list[int]), estimated_time_remaining
- **VideoAbortResult**: media_id, status, cleaned_up, chunks_removed, s3_cleanup
- **ChunkRetryResult**: chunk_number, upload_url, expires_at, retry_attempt, max_retries
- **BatchVideoUploadSession**: batch_id, videos (list[VideoUploadSession]), total_videos, total_size_bytes, estimated_time_seconds, processing_priority
- **VideoAnalysisJobStatus**: analysis_job_id, media_id, status, progress_percent, current_step, started_at, completed_at, error_message, error_code, retry_count, max_retries, estimated_completion_time. Properties: is_completed, is_failed, is_pending, is_terminal (bool)
- **VideoAnalysisRetryResult**: analysis_job_id, media_id, status, retry_count, max_retries, message
- **VideoAnalysisQueueResult**: analysis_job_id, media_id, queue_position, estimated_start_time, message
- **VideoScene**: scene_id, scene_index, start_time, end_time, time_range_formatted, description, tags (list[str]), visible_text, confidence_score, embedding_status, thumbnail_url, created_at
- **VideoSceneList**: scenes (list[VideoScene]), count (int), video_id
- **VideoUploadStatus** (enum): UPLOADING, PROCESSING, COMPLETED, FAILED, ABORTED
- **VideoAnalysisStatus** (enum): PENDING, PROCESSING, COMPLETED, FAILED, CANCELLED
- **VideoChunkProgressEvent**: chunk_number, total_chunks, bytes_uploaded, total_bytes, media_id, filename. Property: progress_percent
- **VideoUploadCompleteEvent**: media_id, result (VideoUploadResult)
- **VideoAnalysisProgressEvent**: job_status (VideoAnalysisJobStatus). Property: progress_percent
...
| POST | /video-uploads/batch/initiate | Batch initiate for up to 10 videos |
...
| POST | /video-uploads/chunks/retry | Get new presigned URL for failed chunk |
| POST | /video-uploads/abort | Cancel upload, clean up resources |
| GET | /video-analysis/jobs/{job_id} | Get analysis job status |
| GET | /video-analysis/status/{media_id} | Get latest analysis status for a video |
| POST | /video-analysis/jobs/{job_id}/retry | Retry failed analysis job |
| POST | /video-analysis/queue | Manually queue video for analysis |
| GET | /videos/{id}/scenes | List AI-detected scenes |
| GET | /video-scenes/{scene_id}/thumbnail | Get scene thumbnail |
